                            ...
                            `).join("");const ne=document.getElementById("pop-play__caro-info-summary2--id");ne&&R.sports_allowed&&Array.isArray(R.sports_allowed)&&(ne.innerHTML=R.sports_allowed.map(M=>{const z=M.key||M,re=Ft.find(Te=>Te.key===z),ee=re?re.group:"default";return`<bma-sport-icon sport="${z}" data-sport-group="${ee}"></bma-sport-icon>`}).join(""));const X=document.getElementById("pop-play__caro-info-list--id");X&&R.matches_expanded&&Array.isArray(R.matches_expanded)&&(X.innerHTML=R.matches_expanded.map(M=>{const z=M.scoreboard_data?typeof M.scoreboard_data=="string"?M.scoreboard_data:JSON.stringify(M.scoreboard_data):"";return`
                                    ...
                                    `}).join(""))})(),setTimeout(()=>{const A=h.appSession?.session_user?.guid;d&&ve.fetchLeaderboard(d).then(R=>{const U=document.getElementById("pop-play__caro-leaderboard--id");if(!U)return;const P=R?.data||R?.rows;if(P&&P.length>0){const V=h.coreTourn[h.coreTourn.length-1]?.data.find(ee=>ee.guid===d);U.innerHTML=P.map((ee,Te)=>{let de="";if(V?.tags&&Array.isArray(V.tags)){const xe=V.tags.find(ue=>typeof ue=="object"&&ue!==null&&ue[ee.user_guid]?ue[ee.user_guid].startsWith("--badge__ribbon--"):!1);if(xe){const ue=xe[ee.user_guid];ue==="--badge__ribbon--gold"?de="badge-trophy--gold":ue==="--badge__ribbon--silver"?de="badge-trophy--silver":ue==="--badge__ribbon--bronze"&&(de="badge-trophy--bronze")}}const Le=ee.combined_betslips?typeof ee.combined_betslips=="string"?ee.combined_betslips:JSON.stringify(ee.combined_betslips):"[]";let Se="[]";if(ee.combined_betslips&&V?.matches_expanded){const xe=typeof ee.combined_betslips=="string"?JSON.parse(ee.combined_betslips):ee.combined_betslips,ue=[...new Set(xe.map(K=>K.coreMatches__guid||K.match_guid||K.odds_id).filter(Boolean))],k=V.matches_expanded.filter(K=>{const Y=K.guid||K.id||K.odds_id;return ue.includes(Y)});Se=JSON.stringify(k)}return`
                                                <bma-leaderboard-card
                                                    ...
                                                    data-sync-theme="dark"
                                                ></bma-leaderboard-card>
                                            `}).join("");const ne=P.findIndex(ee=>ee.user_guid===A),X=ne>=0?ne+1:P.length,M=V?.entities?.guids?.length||P.length,z=document.getElementById("dashboard-rank"),re=document.getElementById("dashboard-rank-percentile");if(z&&(z.innerHTML=`
