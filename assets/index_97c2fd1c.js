                            ...
                            `).join("");const P=document.getElementById("pop-play__caro-info-summary2--id");P&&x.sports_allowed&&Array.isArray(x.sports_allowed)&&(P.innerHTML=x.sports_allowed.map($=>{const ne=$.key||$,Z=Kn.find(Q=>Q.key===ne),j=Z?Z.group:"default";return`<bma-sport-icon sport="${ne}" data-sport-group="${j}"></bma-sport-icon>`}).join(""));const V=document.getElementById("pop-play__caro-info-list--id");V&&x.matches_expanded&&Array.isArray(x.matches_expanded)&&(V.innerHTML=x.matches_expanded.map($=>{const ne=$.scoreboard_data?typeof $.scoreboard_data=="string"?$.scoreboard_data:JSON.stringify($.scoreboard_data):"";return`
                                    ...
                                    `}).join(""))})(),setTimeout(()=>{const w=d.appSession?.session_user?.guid;c&&be.fetchLeaderboard(c).then(x=>{const D=document.getElementById("pop-play__caro-leaderboard--id");if(!D)return;const O=x?.data||x?.rows;if(O&&O.length>0){const Y=d.coreTourn[d.coreTourn.length-1]?.data.find(P=>P.guid===c);D.innerHTML=O.map((P,V)=>{let $="";if(Y?.tags&&Array.isArray(Y.tags)){const Z=Y.tags.find(j=>typeof j=="object"&&j!==null&&j[P.user_guid]?j[P.user_guid].startsWith("--badge__ribbon--"):!1);if(Z){const j=Z[P.user_guid];j==="--badge__ribbon--gold"?$="badge-trophy--gold":j==="--badge__ribbon--silver"?$="badge-trophy--silver":j==="--badge__ribbon--bronze"&&($="badge-trophy--bronze")}}const ne=P.combined_betslips?typeof P.combined_betslips=="string"?P.combined_betslips:JSON.stringify(P.combined_betslips):"[]";return`
                                                <bma-leaderboard-card
                                                    ...
                                                    data-badge-class="${$}"
                                                    data-is-current-user="${P.user_guid===w}"
                                                    data-sync-theme="dark"
                                                ></bma-leaderboard-card>
