                            ...
                            `).join("");const ne=document.getElementById("pop-play__caro-info-summary2--id");ne&&R.sports_allowed&&Array.isArray(R.sports_allowed)&&(ne.innerHTML=R.sports_allowed.map(B=>{const z=B.key||B,ie=sn.find(Te=>Te.key===z),ee=ie?ie.group:"default";return`<bma-sport-icon sport="${z}" data-sport-group="${ee}"></bma-sport-icon>`}).join(""));const X=document.getElementById("pop-play__caro-info-list--id");X&&R.matches_expanded&&Array.isArray(R.matches_expanded)&&(X.innerHTML=R.matches_expanded.map(B=>{const z=B.scoreboard_data?typeof B.scoreboard_data=="string"?B.scoreboard_data:JSON.stringify(B.scoreboard_data):"";return`
                                    ...
                                    `}).join(""))})(),setTimeout(()=>{const A=h.appSession?.session_user?.guid;d&&ve.fetchLeaderboard(d).then(R=>{const U=document.getElementById("pop-play__caro-leaderboard--id");if(!U)return;const k=R?.data||R?.rows;if(k&&k.length>0){const V=h.coreTourn[h.coreTourn.length-1]?.data.find(ee=>ee.guid===d);U.innerHTML=k.map((ee,Te)=>{let fe="";if(V?.tags&&Array.isArray(V.tags)){const Ee=V.tags.find(ye=>typeof ye=="object"&&ye!==null&&ye[ee.user_guid]?ye[ee.user_guid].startsWith("--badge__ribbon--"):!1);if(Ee){const ye=Ee[ee.user_guid];ye==="--badge__ribbon--gold"?fe="badge-trophy--gold":ye==="--badge__ribbon--silver"?fe="badge-trophy--silver":ye==="--badge__ribbon--bronze"&&(fe="badge-trophy--bronze")}}const He=ee.combined_betslips?typeof ee.combined_betslips=="string"?ee.combined_betslips:JSON.stringify(ee.combined_betslips):"[]";return`
                                                <bma-leaderboard-card
                                                    ...
                                                    data-combined-betslips="${He.replace(/"/g,"&quot;")}"
                                                    data-badge-class="${fe}"
                                                    data-is-current-user="${ee.user_guid===A}"
                                                    data-sync-theme="dark"
                                                ></bma-leaderboard-card>
                                            `}).join("");const ne=k.findIndex(ee=>ee.user_guid===A),X=ne>=0?ne+1:k.length,B=V?.entities?.guids?.length||k.length,z=document.getElementById("dashboard-rank"),ie=document.getElementById("dashboard-rank-percentile");if(z&&(z.innerHTML=`
                                                <span class="tourn-dashboard__rank-text">${X}/${B}</span>
                                            `),ie&&B>0){const ee=Math.round((B-X+1)/B*100);ie.textContent=`Top ${ee}%`}}else U.innerHTML=`
